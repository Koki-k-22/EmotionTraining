...
- **100本ノック（質問生成）**: `questions_knock_v1.json`（10ペルソナ×5話題=50回答文、各回答文に
  8型×3例=1200の模範質問。docs/AUTHORING_KNOCK.md 準拠）。`app/js/knock.js` の `expandKnock` で
  (回答文×型)=400 の仮想問題に展開し、`buildKnockSessionIds` が1セッション100問（各回答文2型）を組む。
  フラッシュカード形式（頭の中で質問を作る→答えを見る→次へ。採点・記録なし、テンポ優先）。
  復習・型別成績の対象外。ルートは `#knock`。2026-08-28 追加、08-29 フラッシュカード化、08-30 模範3例化+採点廃止
